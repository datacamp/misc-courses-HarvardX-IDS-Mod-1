...
murders["population"]
```
R returns a subset of the original data frame containing just this column. This new ojbect will be of class `data.frame` rather than a vector. To access the column itself you need to use either the `$` accessor or the double square brackets `[[`. 
...
`@instructions`
For this question we will combine two functions, in nested way, to determine the number of unique categories.
