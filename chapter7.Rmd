...
# Use filter to create a new dataframe `no_south`
...
`@instructions`
Repeat exercise 5, but now instead of creating a new object, show the result and only include the state, rate, and rank columns. Use a pipe (`%>%`) to do this in just one line.
...
`@instructions`
Now reload the original murders data frame using `data(murders)`. Then use just one line to create a new data frame, called, `my_states` that has murder rate and rank column, consider only states in the Northeast or West, which have a murder rate lower than 1 and contain only the state, rate, and rank columns. 
