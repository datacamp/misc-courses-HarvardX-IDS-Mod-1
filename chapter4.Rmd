...
Remember that when we extract a varibale from a data frame the order of the resulting vector is the same as the order of the rows of the data frame. 
...
We previously computed the avearge of `na_example` using `mean(na_example)` and obtain `NA`. This is because the function `mean` returns `NA` if it encounters at least one `NA`.
A common operation is therefore emoving the entries that are NA and after that perform operations on the rest. 
