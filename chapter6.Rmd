...
`@instructions`
We've stored the *murders* object in memory.
...
`@instructions`
Now extend the code from the previous exercises to report the states in the Northeast with murder rates lower than 1.
...
`@instructions`
Use the `%in%` operator to create a logical vector that answers the following question.
