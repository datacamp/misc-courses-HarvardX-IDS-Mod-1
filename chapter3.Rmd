...
`@instructions`
The temperatures listed are from Beijing, Lagos, Paris, Rio de Janeiro, San Juan, and Toronto.
...
We can avoid issues with coercion in R by changing characters to integers and vice-versa. This is known as typecasting. The code, `as.integer(x)` helps us convert character strings to integers. There is an equivalent function that converts integers to strings, `as.character(x)`.
...
- Notice that `x` has been coerved into a character string.
- Convert `x` to a vector of integers using the `as.integer()` function.
