...
This can be useful becuase sometimes all we want are sequential numbers  that are smaller than some value. 
...
# It simply determines the maximun value permited.
...
The `sequence` function has another useful argument. The argument *length.out*. This argument let's us generate sequences that are increasing by the same amount but are of the prespecified length. 
...
`@instructions`
- We will define the class of a vector generated with `seq` using the *length.out* argument. 
...
- Coerce `x` to get integers using the `as.integer()` function.
